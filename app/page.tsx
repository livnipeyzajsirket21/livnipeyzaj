...
export default function Home() {
  ...
  const trackConversionClick = (event: React.MouseEvent<HTMLAnchorElement>, url: string) => {
    event.preventDefault();
    const win = window as any;
    if (typeof win.gtag_report_conversion === "function") {
      win.gtag_report_conversion(url);
    } else {
  ...
  const whatsappNumber = "905322251185";
  const whatsappMessage = "Merhaba, ücretsiz keşif için bilgi almak istiyorum.";
  const openWhatsAppQuick = () => {
    const url = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(whatsappMessage)}`;
    window.open(url, "_blank", "noopener,noreferrer");
    setTeklifFormAcik(false);
  };
  const callPhoneQuick = () => {
    window.location.href = "tel:+905322251185";
    setTeklifFormAcik(false);
  };
  ...
  return (
      <main className="min-h-screen font-sans">
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 flex flex-col gap-6">
                ...
                {/* 2'li İletişim Kartları Grid (E-Posta, Adres) */}
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="bg-[#183325] rounded-[2rem] p-10 flex flex-col items-center text-center justify-center min-h-[340px]">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    onClick={(event) => trackConversionClick(event, "https://wa.me/905322251185")}
                    className="bg-[#25d366] text-white font-bold w-full py-4 rounded-xl hover:bg-[#20b858] transition flex items-center justify-center gap-2 text-[15px] shadow-lg shadow-[#25d366]/20"
                  >
        ...
        {/* ÜCRETSİZ KEŞİF KISAYOL MODALİ */}
        {mounted && teklifFormAcik && (
          <div
            className="fixed inset-0 z-[150] flex items-center justify-center p-4 sm:p-8 bg-black/60 backdrop-blur-sm"
            role="dialog"
            aria-modal="true"
            aria-label="Ücretsiz Keşif Randevusu"
            onClick={() => setTeklifFormAcik(false)}
          >
            <div
              className="bg-white rounded-3xl max-w-md w-full shadow-2xl my-8 p-6"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h2 className="text-xl sm:text-2xl font-bold text-[#183325]">Ücretsiz Keşif Randevusu</h2>
                  <p className="text-gray-600 mt-2 text-sm">Ankara geneli ücretsiz keşif ve projelendirme için bizimle nasıl iletişime geçmek istersiniz?</p>
                </div>
                <button
                  type="button"
                  onClick={() => setTeklifFormAcik(false)}
              ...
              <div className="mt-6 flex flex-col gap-3">
                <button
                  ...
                  className="w-full rounded-xl bg-[#25D366] text-white py-4 font-semibold flex items-center justify-center gap-3 shadow-lg"
                >
                  <svg className="w-6 h-6" viewBox="0 0 24 24" fill="white" aria-hidden>
                    <path d="M20.52 3.48A11.91 11.91 0 0012 .01 11.93 11.93 0 001.48 9.99c0 2.11.55 4.17 1.6 6.01L0 24l6.3-1.65a11.97 11.97 0 005.69 1.45c6.63 0 12-5.37 12-12 0-3.2-1.25-6.18-3.48-8.52zM17.2 15.1c-.3-.15-1.76-.9-2.03-.99-.27-.09-.46-.14-.65.15-.19.29-.71.94-.87 1.13-.16.19-.33.21-.61.07-.28-.15-1.18-.43-2.25-1.39-.84-.74-1.4-1.62-1.56-1.9-.16-.28-.02-.43.12-.58.12-.12.27-.31.41-.47.14-.17.18-.28.27-.47.09-.19.04-.33-.02-.47-.06-.14-.54-1.34-.74-1.84-.2-.49-.4-.41-.54-.42-.14 0-.31-.01-.48-.01-.17 0-.4.06-.61.29-.21.22-.78.82-.78 1.99 0 1.17.88 2.37 1.01 2.54.13.18 2.24 3.63 5.43 5.08 3.19 1.45 3.19 1 3.76.94.57-.06 1.81-.93 2.07-1.63.26-.7.26-1.22.18-1.35-.08-.13-.29-.2-.6-.35z" />
                  </svg>
                  <span className="text-left">WhatsApp ile İletişime Geç</span>
                </button>

                <button
                  type="button"
                  onClick={callPhoneQuick}
                  className="w-full rounded-xl bg-[#183325] text-white py-4 font-semibold flex items-center justify-center gap-3 shadow-lg"
                >
                  <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
                    <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.86 19.86 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6A19.86 19.86 0 0 1 2.08 4.18 2 2 0 0 1 4 2h3a2 2 0 0 1 2 1.72c.12.9.37 1.76.73 2.57a2 2 0 0 1-.45 2.11L9.91 9.91a16 16 0 0 0 6 6l1.5-1.5a2 2 0 0 1 2.11-.45c.81.36 1.67.61 2.57.73A2 2 0 0 1 22 16.92z" />
                  </svg>
                  <span className="text-left">Telefon ile Ara</span>
                </button>
              </div>
            </div>
          </div>
